...
\end{equation}
また，出力層では単純に仮想粒子の分布$f(\bm{x}, \bm{v}, t_1)$から式(\ref{todo})を用いて流速$\bm{u}(\bm{x}, t_1)$と圧力$\rho(\bm{x}, t_1)$を算出した．これらと実際の流速$\hat{\bm{u}}(\bm{x}, t_1)$と圧力$\hat{\rho}(\bm{x}, t_1)$から求められる二乗損失は，式(\ref{todo})から次のように表される．
\begin{equation}
  E = \sum_{\bm{x}} 
