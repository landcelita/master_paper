...
本実験では，提案モデルと\ref{subsection:exp-u-net-like-model}項で説明するU-Net-likeなモデルの2つのモデルに3時刻分の時系列データを入力し，それぞれの3時間後の風速を予測するように訓練した．その学習には，\ref{section:exp-data-and-condition}節で述べるように気象庁が提供している日本近辺の風速と気圧データを用いた．
...
\begin{enumerate}
  \item 全体の風速のRMSE(Root Mean Squared Error, 二乗平均平方根誤差)と提案モデルのRMSE改善率
  \item 全体の風速のME(Mean Error, 平均誤差)
  \item 全体の風向のRMSEと提案モデルのRMSE改善率
  \item 全体の風向のME
  \item 座標ごとの風速のRMSEと提案モデルのRMSE改善率
  \item 座標ごとの風速のME
  \item 座標ごとの風向のRMSEと提案モデルのRMSE改善率
  \item 座標ごとの風向のME
...
\end{equation}
\begin{equation}
  (\mathrm{全体の風向のME [^\circ]}) = \frac{1}{NM} \sum_{\bm{x}, i} \mathrm{arg}(\bm{w}(\bm{x}, t_i), \hat{\bm{w}}(\bm{x}, t_i))
  \label{eq:exp-me-direction}
\end{equation}
\begin{equation}
  (\mathrm{座標ごとの風速のRMSE [m/s]}) = \sqrt{\frac{1}{N} \sum_{\bm{x}} (|\bm{w}(\bm{x}, t_i)| - |\hat{\bm{w}}(\bm{x}, t_i)|)^2}
  \label{eq:exp-rmse-per-point}
\end{equation}
\begin{equation}
  (\mathrm{座標ごとの風速のME [m/s]}) = \frac{1}{N} \sum_{\bm{x}} (|\bm{w}(\bm{x}, t_i)| - |\hat{\bm{w}}(\bm{x}, t_i)|)
  \label{eq:exp-me-per-point}
\end{equation}
\begin{equation}
  (\mathrm{座標ごとの風向のRMSE [^\circ]}) = \sqrt{\frac{1}{N} \sum_{\bm{x}} \mathrm{arg}(\bm{w}(\bm{x}, t_i), \hat{\bm{w}}(\bm{x}, t_i))^2}
  \label{eq:exp-rmse-direction-per-point}
\end{equation}
\begin{equation}
  (\mathrm{座標ごとの風向のME [^\circ]}) = \frac{1}{N} \sum_{\bm{x}} \mathrm{arg}(\bm{w}(\bm{x}, t_i), \hat{\bm{w}}(\bm{x}, t_i))
  \label{eq:exp-me-direction-per-point}
\end{equation}
...
(前日の)21:00, 0:00, 3:00の3時刻分のデータを入力として0:00, 3:00, 6:00の風速を予測させた．すなわち，式(\ref{eq:time-series-input})において$n=4$，式(\ref{eq:time-series-t})において無次元時刻$\Delta T$は$3[\mathrm{h}]$相当である． %todo
